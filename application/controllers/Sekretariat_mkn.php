...
class Sekretariat_mkn extends CI_Controller {
    ...
    public function __construct() {
        ...
        $allowed = array('sek_mkn', 'sekretariat_mkn', 'superadmin');
        if (!in_array($level, $allowed)) { redirect('login'); }

    }

    // Dashboard Sekretariat
    public function index() {
        $data['user'] = $this->db->get_where('tbl_user', array('id_user' => $this->session->userdata('id_user')));
        $data['kpi'] = isset($data['kpi']) ? $data['kpi'] : array();
